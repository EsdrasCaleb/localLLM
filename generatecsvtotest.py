...
# 1. Carregar e filtrar os CSVs
all_files = glob.glob(os.path.join(csv_folder, "benchmarkresults_*.csv"))


# Função para corrigir apenas as linhas que têm ")" seguido de ","
def fix_csv_format(file_path):
    ...
    fixed_lines = []
    for line in lines:
        if re.search(r"\),", line):  # Só processa se encontrar ") ,"
            line = re.sub(r"\)(,)", r')"\1', line, count=1)
            line = re.sub(r"^(.*?,.*?),", r'\1,"', line, count=1)
        fixed_lines.append(line)

    # Sobrescreve o arquivo corrigido
    with open(file_path, "w", encoding="utf-8") as file:
        file.writelines(fixed_lines)

# Corrige todos os arquivos CSV
for csv_file in all_files:
    fix_csv_format(csv_file)



# Função para extrair o root_path
...
# 2. Gerar CSV intermediário
df_result[['project', 'file','source_file']].to_csv(output_csv, index=False, header=False)

# 3. Rodar TestSmellDetector.jar e esperar o processo terminar
subprocess.run(["java", "-jar", jar_path, output_csv], check=True)

# 4. Buscar a saída do TestSmellDetector (pegar o primeiro arquivo encontrado)
output_files = glob.glob("Output_TestSmellDetection_*.csv")
if output_files:
    output_test_smell = output_files[0]  # Pega o primeiro encontrado
...
# 5. Análises estáticas com Lizard e contagem de asserts
...
# 6. Criar dataframe_final sem prompt, class e method
df_final = df_result.drop(columns=['prompt', 'class', 'method'], errors='ignore')

backup_csv = "backup_results2.csv"

# 1. Salvar backup localmente antes de enviar ao Google Drive
