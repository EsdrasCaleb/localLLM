...
def generate_prompt(messages,model_name):
    ...
    "deepseek-ai/deepseek-coder-1.3b-instruct","deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B","deepseek-ai/deepseek-coder-6.7b-instruc","tiiuae/Falcon3-1B-Instruct"] or
...
def generate_model(prompt,model_name,temperature,max_tokens):
    ...
    #return models[model_name](prompt,temperature=temperature,max_new_tokens=max_tokens,return_full_text=False,do_sample=True)[0]['generated_text']
    if model_name in ["google/gemma-2-2b-it"]:
        inputs = tokenizers[model_name](prompt, return_tensors="pt", padding=True, truncation=True,max_length=8192)
